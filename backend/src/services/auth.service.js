...
async function registerUser({ nombre, email, password }) {
  ...
  const [result] = await pool.execute('INSERT INTO usuarios (nombre, email, password, rol) VALUES (?, ?, ?, ?)', [nombre, email, hash, 'cliente']);
  return { id_usuario: result.insertId };
}
async function loginUser({ email, password }) {
  if (!email || !password) throw new AppError('email y password son obligatorios', 400);
  const [rows] = await pool.execute('SELECT id_usuario, nombre, email, password, rol FROM usuarios WHERE email = ?', [email]);
  if (!rows.length) throw new AppError('Credenciales incorrectas', 401);
  const user = rows[0];
  if (!(await bcrypt.compare(password, user.password))) throw new AppError('Credenciales incorrectas', 401);
