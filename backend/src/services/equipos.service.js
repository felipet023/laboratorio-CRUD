...
async function borrarImagenSiExiste(imagen) {
  if (!imagen) return;
  try { await fs.unlink(path.join(UPLOAD_DIR, imagen)); } catch {}
}
...
async function getEquipoById(id) {
  ...
  if (!rows.length) throw new AppError('Equipo no encontrado', 404);
  return rows[0];
}
async function createEquipo({ nombre, marca, modelo }, imagen) {
  if (!nombre) throw new AppError('nombre es obligatorio', 400);
  const [result] = await pool.execute('INSERT INTO equipos (nombre, marca, modelo, imagen) VALUES (?, ?, ?, ?)', [nombre, marca || null, modelo || null, imagen || null]);
...
async function updateEquipo(id, { nombre, marca, modelo }, imagen) {
  ...
  const [result] = await pool.execute('UPDATE equipos SET nombre = ?, marca = ?, modelo = ?, imagen = ? WHERE id_equipo = ?', [nombre, marca || null, modelo || null, nuevaImagen, id]);
  if (!result.affectedRows) throw new AppError('Equipo no encontrado', 404);
  if (imagen && actual.imagen && actual.imagen !== imagen) await borrarImagenSiExiste(actual.imagen);
}
