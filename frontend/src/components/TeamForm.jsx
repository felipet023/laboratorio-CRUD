...
export default function TeamForm({ editing, onSubmit, onCancel }) {
  ...
  useEffect(() => {
    setForm(editing || empty); setImagenFile(null);
  ...
  function changeImagen(event) { const file = event.target.files?.[0]; if (!file) return; setImagenFile(file); setPreview(URL.createObjectURL(file)); }
  function submit(event) { event.preventDefault(); onSubmit({ ...form, imagenFile }); setForm(empty); setImagenFile(null); setPreview(null); if (fileInputRef.current) fileInputRef.current.value = ''; }
  return (
    <form onSubmit={submit} className="card form-grid">
      ...
      {preview && <div className="image-preview"><img src={preview} alt="Vista previa" /></div>}
      <div className="actions">
        <button type="submit" className={editing ? 'edit' : ''}>{editing ? <Save size={16} /> : <Plus size={16} />}{editing ? 'Actualizar' : 'Crear'}</button>
        {editing && <button type="button" className="secondary" onClick={onCancel}><X size={16} />Cancelar</button>}
      </div>
    </form>
  );
}
